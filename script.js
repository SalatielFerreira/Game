...
const btnLoseMenu = document.getElementById('btn-lose-menu');
...
function salvarDados() {
    // Certifica-se de que startTime é salvo como string se não for null
    const currentStartTime = startTime ? startTime.toString() : null;
    const estado = { 
        scoreTotal, 
        acertosSeguidos, 
        maxTentativas, 
        ...
        palpiteAtual, 
        tempoSegundos, 
        startTime: currentStartTime, // Salva como string
        jogoAtivo, 
        historicoHTML: listaHistorico.innerHTML, 
        tabuleiroHTML: gameBoard.innerHTML, 
...
function carregarDados() {
    ...
    if (salvo) {
        ...
        streakDisplay.innerText = acertosSeguidos;
        listaHistorico.innerHTML = salvo.historicoHTML;
        
        // Ativa o botão de dificuldade correto
        ...
        if (salvo.jogoEmAndamento === 'true') {
            ...
            // Continua o timer apenas se o jogo estiver ativo e startTime for válido
            if (jogoAtivo && salvo.startTime) {
                startTime = parseInt(salvo.startTime); // Converte de volta para número
                continuarTimer();
            } else {
                atualizarDisplayTimer(); // Atualiza o display com o tempo salvo se o jogo não estiver ativo
                clearInterval(intervaloTimer); // Garante que o timer não esteja rodando sem necessidade
            }
...
function initGame() {
    ...
    jogoAtivo = true; 
    startTime = Date.now(); 
    localStorage.setItem('startTime', startTime.toString()); // Salva como string
    tempoSegundos = 0;
    
    messageDisplay.innerHTML = ""; 
    btnNext.style.display = "none"; 
    btnLoseMenu.style.display = "none";
    ...
    for (let i = 0; i < maxTentativas; i++) {
        ...
        for (let j = 0; j < 5; j++) { 
            ...
            tile.className = 'tile'; 
            tile.id = `tile-${i}-${j}`; 
            tile.onclick = () => selecionarTile(i, j); 
...
function selecionarTile(r, c) { 
    if (!jogoAtivo || r !== tentativaAtual) return; 
    letraAtual = c; 
    atualizarDestaque(); 
}
...
// Processa as teclas pressionadas no teclado virtual ou físico
function lidarComClique(t) {
    ...
    if (t === "Apagar") { 
        ...
        for(let i=0; i<5; i++) {
            p += tiles[i].innerText;
            if (!tiles[i].innerText) { // Se algum tile estiver vazio, não permite enviar
                messageDisplay.innerText = "Complete a palavra!"; 
                setTimeout(() => messageDisplay.innerText = "", 1500);
                return;
            }
        }
        if (p.length === 5) { 
    ...
    } else if (t.length === 1 && /^[A-Z]$/.test(t)) { // Verifica se é uma letra maiúscula
        if (letraAtual < 5) { // Garante que não ultrapassa 5 letras
...
function validarPalpite() {
    let sec = palavraSecreta.split('');
    let pal = palpiteAtual.split('');
    const row = gameBoard.children[tentativaAtual];
    const tiles = row.children;
    
    let tempPalavraSecreta = [...sec]; // Cópia para não modificar o original
    let avaliacoes = Array(5).fill(''); // Para armazenar 'correct', 'present', 'absent'

    // Primeira passada: marca letras corretas na posição correta
    for (let i = 0; i < 5; i++) {
        if (pal[i] === tempPalavraSecreta[i]) {
            avaliacoes[i] = 'correct';
            tempPalavraSecreta[i] = null; // Marca como usada
        }
    }

    // Segunda passada: marca letras que existem mas em posição errada ou ausentes
    for (let i = 0; i < 5; i++) {
        if (avaliacoes[i] === '') { // Se ainda não foi marcada como 'correct'
            let idx = tempPalavraSecreta.indexOf(pal[i]);
            if (idx > -1) {
                avaliacoes[i] = 'present';
                tempPalavraSecreta[idx] = null; // Marca como usada
            } else {
                avaliacoes[i] = 'absent';
            }
        }
    ...
    // Aplica as classes e atualiza o teclado
    for (let i = 0; i < 5; i++) {
        const tile = tiles[i];
        const letter = pal[i];
        const status = avaliacoes[i];

        // Adiciona um pequeno delay para a animação dos tiles
        setTimeout(() => {
            tile.classList.add(status);
            atualizarTecla(letter, status);
        }, i * 100); // 100ms de delay entre cada tile
    }

    // Verifica vitória ou derrota após o último tile ser animado
    setTimeout(() => {
        if (palpiteAtual === palavraSecreta) {
            jogoAtivo = false; 
            tempoSegundos = Math.floor((Date.now() - parseInt(localStorage.getItem('startTime'))) / 1000);
            vitoria();
        } else {
            tentativaAtual++; 
            letraAtual = 0; 
        ...
        salvarDados();
    }, 5 * 100 + 200); // Espera a animação de todos os tiles + um pequeno extra
}
...
function vitoria() { 
    ...
    clearInterval(intervaloTimer); // Para o timer na vitória
}
...
function derrota() { 
    ...
    clearInterval(intervaloTimer); // Para o timer na derrota
}

// Exibe mensagem de vitória/derrota e botões de ação
function mostrarBotoesFinais(v) {
    if (v) { 
        messageDisplay.innerHTML = `<div class="msg-status" style="color:var(--cor-correta)">VENCEU!</div><div class="msg-palavra">Palavra Chave: ${palavraSecreta}</div>`; 
        btnNext.style.display = "inline-block"; 
    } else { 
        messageDisplay.innerHTML = `<div class="msg-status" style="color:var(--cor-perigo)">FIM DE JOGO!</div><div class="msg-palavra">Palavra Chave: ${palavraSecreta}</div>`; 
        btnLoseMenu.style.display = "inline-block"; 
    }
    scoreDisplay.innerText = scoreTotal; 
    streakDisplay.innerText = acertosSeguidos;
    atualizarDestaque(); // Remove o destaque do tile selecionado
}
...
function continuarTimer() { 
    ...
    intervaloTimer = setInterval(() => { 
        if (jogoAtivo) { 
            tempoSegundos = Math.floor((Date.now() - startTime) / 1000); 
            atualizarDisplayTimer(); 
        } else {
            clearInterval(intervaloTimer); // Para o timer se o jogo não estiver ativo
        }
    }, 1000); 
    atualizarDisplayTimer(); 
}
...
function atualizarDisplayTimer() { 
    let total = jogoAtivo ? Math.floor((Date.now() - startTime) / 1000) : tempoSegundos; 
    let min = Math.floor(total / 60).toString().padStart(2, '0'); 
    let seg = (total % 60).toString().padStart(2, '0'); 
    timerDisplay.innerText = `${min}:${seg}`; 
}

// Sincroniza as cores das teclas do teclado com o que já foi tentado no tabuleiro
function sincronizarTeclado() { 
    // Limpa todas as classes de cor do teclado primeiro
    ...
    document.querySelectorAll('.tile').forEach(t => { 
        ...
        if (letter) { // Apenas se o tile tiver uma letra
            if (t.classList.contains('correct')) atualizarTecla(letter, 'correct'); 
            else if (t.classList.contains('present')) atualizarTecla(letter, 'present'); 
            else if (t.classList.contains('absent')) atualizarTecla(letter, 'absent'); 
        }
    }); 
}
...
function gerarTeclado() { 
    ...
    sincronizarTeclado(); // Sincroniza logo após gerar o teclado
}
...
function registrarNoHistorico(vitoria) { 
    ...
    item.innerHTML = `<span class="${vitoria ? 'hist-vitoria' : 'hist-derrota'}">${vitoria ? '✓' : '✗'} ${palavraSecreta}</span><br><small style="color:#666">⏱ ${timerDisplay.innerText} | 🎯 ${tentativaAtual + (vitoria?1:0)}/${maxTentativas}</small>`; 
